    ...
    try {
        ...
        figurinhas.forEach(figurinha => {
            const slot = Array.from(slots).find(s => {
                const numEl = s.querySelector(".slot-number");
                if (numEl) {
                    const num = parseInt(numEl.textContent.replace("#", ""), 10);
                    return num === figurinha.id;
                }
                return false;
            });

            if (slot) {
                // Evita duplicar se já existir a imagem
                if (!slot.querySelector(".sticker-img")) {
        ...
        console.error("Erro ao preencher as figurinhas:", error);
    }
}
